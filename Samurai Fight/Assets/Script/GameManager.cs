...
public class GameManager : MonoBehaviour
{
    ...
    // DIUBAH: Saat pemain menekan tombol aksi yang butuh kartu, panel kartu akan muncul
    ...
    private IEnumerator ExecutePlayerSerangBalikCoroutine()
    {
        ...
        uiManager.ShowMessage("Pilih 1 kartu untuk SERANG BALIK.", 0f);
        if (uiManager.KartuManusiaPanel != null) uiManager.KartuManusiaPanel.SetActive(true); // Tampilkan kartu
        uiManager.SetPlayerHandInteractable(true);
    }

    // --- Sisa skrip tidak berubah ---
    #region Unchanged GameManager Code
    ...
    private void InitiateAttack(Player currentAttacker, Player currentDefender, int value, bool isCounter)
    {
        ...
        else
        {
            ...
            string message = isCounter ? $"Anda diserang balik! Kekuatan: {value}." : $"Anda diserang! Kekuatan: {value}.";
            uiManager.ShowMessage(message + " Tangkis?", 0f);
    ...
    private IEnumerator ExecuteAISerangBalikCoroutine()
    {
        if (ai.hand.Count == 0)
        {
            StartCoroutine(RoundOverCoroutine(manusia));
            yield break;
        }
    ...
    private IEnumerator ExecuteTieBreakerCoroutine()
    {
        uiManager.HideAllPlayerPanels();
        uiManager.HideAttackStrength();
        uiManager.ShowMessage("Pemain terpojok atau dek habis! TIE BREAKER!", 2.5f);
        yield return new WaitForSeconds(2.5f);
